...
export default function App() {
  ...
  const lodging = useMemo(() => {
    if (isOneDay) return 0;
    return Number(cost.lodging || 0);
  }, [cost.lodging, isOneDay]);
      ...
      <div style={wrap} ref={ref}>
        ...
        <Section title="기본 정보">
          <Grid>
            ...
            <Select label="교통수단" value={form.transport}
              options={[
                "자동차",
                "항공",
                "기차",
        ...
        <Buttons>
          <button onClick={saveImage}>이미지</button>
...
const Header = () => (
  <div style={header}>출장비 자동 정산 시스템</div>
);
...
const SummaryRow = ({ children }) => (
  <div style={summary}>{children}</div>
);

const Buttons = ({ children }) => (
  <div style={btnWrap}>{children}</div>
);

const Info = ({ children }) => (
  <div style={info}>{children}</div>
);
...
const titleStyle = { fontSize: 18, fontWeight: "bold", color: "#1e3a8a" };
...
const info = { fontSize: 12, textAlign: "center", color: "#64748b" };